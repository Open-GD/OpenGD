...
bool AlertLayer::init(std::string title, std::string desc, std::string btn1Str, std::string btn2Str, float width, std::function<void(TextButton*)> btn1Callback, std::function<void(TextButton*)> btn2Callback)
{
	if (!PopupLayer::init()) 
		return false;
	...
	_btn1 = TextButton::create(btn1Str, GameToolbox::getTextureString("goldFont.fnt"), 0, 0, _btnCallback);
	menu->addChild(_btn1);

	if (!btn2Str.empty())
	{
		auto _btn2Callback = btn2Callback ? btn2Callback : [this](TextButton*) {this->close(); };
		_btn2 = TextButton::create(btn2Str, GameToolbox::getTextureString("goldFont.fnt"), 0, 0, _btn2Callback);
		menu->addChild(_btn2);
