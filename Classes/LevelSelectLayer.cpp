...
bool LevelSelectLayer::init()
{
	...
	if (init)
	{
		...
		auto leftArt = ax::Sprite::createWithSpriteFrameName("GJ_sideArt_001.png");
		leftArt->setAnchorPoint({0.0f, 0.0f});
		leftArt->setPosition({0 - 1.0f, 0 - 1.0f});
		addChild(leftArt, 1);

		auto rightArt = ax::Sprite::createWithSpriteFrameName("GJ_sideArt_001.png");
		rightArt->setAnchorPoint({1.0f, 0.0f});
		rightArt->setPosition({569 + 1.0f, 0 - 1.0f});
		rightArt->setFlippedX(true);
		addChild(rightArt, 1);
